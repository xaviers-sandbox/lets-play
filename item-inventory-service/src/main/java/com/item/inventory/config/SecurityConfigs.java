...
public class SecurityConfigs  {
	...
	private static final String[] WHITE_LIST = { "/v1/item-inventories/unsecure-world", "/webjars/**", "/actuator/**",
			"/api-docs", "/api-docs/**", "/swagger-ui.html", "/swagger-ui.html/**", "/swagger-ui/**" };
	
	private static final String[] BLACK_LIST = { "/v1/item-inventories/app", "/v1/item-inventories/app", "/v1/item-inventories/app/**",
			"/v1/item-inventories/dbs", "/v1/item-inventories/dbs/**",
...
// enable security
         httpSecurity
                .authorizeExchange((authorize) -> authorize.pathMatchers(WHITE_LIST)
                        .permitAll()
                        .pathMatchers(BLACK_LIST).hasAnyRole(ROLES_LIST)
                        .anyExchange()
                        .authenticated())
                .csrf(csrf -> csrf.disable())
                .cors(cors -> cors.disable())
